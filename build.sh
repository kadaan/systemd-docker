...
function run() {
  ...
  ${GOX} -os="${XC_OS}" -arch="${XC_ARCH}" -tags 'osusergo netgo static_build' -ldflags "-d -s -w -extldflags \"-fno-PIC -static\" -X github.com/kadaan/systemd-docker/version.Version=$VERSION -X github.com/kadaan/systemd-docker/version.Revision=$REVISION -X github.com/kadaan/systemd-docker/version.Branch=$BRANCH -X github.com/kadaan/systemd-docker/version.BuildUser=$USER -X github.com/kadaan/systemd-docker/version.BuildHost=$HOST -X github.com/kadaan/systemd-docker/version.BuildDate=$BUILD_DATE" -output="dist/{{.Dir}}_{{.OS}}_{{.Arch}}" || fatal "gox failed: $?"
  ...
  if [[ -n "$TRAVIS" ]]; then
    ...
    for f in *; do
      ...
      verbose "   --> $archivename"
      local genericname="systemd-docker$extension"
      mv -f "$f" "$genericname"
